...
for my $tid (0 .. $sam->n_targets - 1)  {
	...
	while (my $pair = $iter_pair->next_seq() ) {
		...
		#my ($oldITR_len, $oldaln_offset, $olduH_len, $oldaln_correction) = find_offset($ref_2,$query_2,$first_mate->target->dna,$second_mate->strand,1);
	
		my ($ITR_len, $uH_len, $fillin_len, $itrCigarString) = getITRdetailsFromAlignment($ref_2,$query_2,$first_mate->target->dna,$second_mate->strand,1);
		...
		## Kevin 07/06/18 - no longer req'd
		...
		my $bamAdditions = "\t".join("\t","it:i:$ITR_len","uh:i:$uH_len","os:i:$fillin_len","mm:i:$nMM","fs:i:$fragSize","ic:Z:$itrCigarString");
		my $bedString    = join("\t" ,$chrom, $start, $end, join("_",$f_qual,$s_qual), join("_",$ITR_len,$uH_len,$fillin_len,$itrCigarString,$fragSize),$strand);

		my $f_line = $first_mate->tam_line().$bamAdditions;
		my $s_line = $second_mate->tam_line().$bamAdditions;


		if ($f_line && $s_line){
			my $lineOut; 

			if ($ITR_len > 5 && $fillin_len > 2 && $uH_len >= 0){
				print OUT_BEDss1 $bedString."\n" ;
				print OUT_SAMss1 $f_line."\n";
				print OUT_SAMss1 $s_line."\n";
				$lineOut++;
			}
			
			if ($ITR_len > 5 && $fillin_len < 3 && $uH_len >= 0){
				print OUT_BEDss2 $bedString."\n" ;
				print OUT_SAMss2 $f_line."\n";
				print OUT_SAMss2 $s_line."\n";
				$lineOut++;
			}

			if ($ITR_len < 3){
				print OUT_BEDds $bedString."\n" ;
				print OUT_SAMds $f_line."\n";
				print OUT_SAMds $s_line."\n";
				$lineOut++;
			}

			if ($ITR_len < 1 && $uH_len < 1){
				print OUT_BEDdsS $bedString."\n" ;
				print OUT_SAMdsS $f_line."\n";
				print OUT_SAMdsS $s_line."\n";
				$lineOut++;
			}

			unless ($lineOut){
				print OUT_BEDunC $bedString."\n" ;
				print OUT_SAMunC $f_line."\n";
				print OUT_SAMunC $s_line."\n";
				$lineOut++;
			}
...
################################################################################
sub getITRdetailsFromAlignment {

	my ($read2Alignment, $read2Seq, $read1Seq, $strand, $maxMismatches) = @_;

	if ($strand == -1) {
		$read2Alignment = scalar reverse $read2Alignment;
		$read2Seq       = scalar reverse $read2Seq;
		$read1Seq      =~ tr/ATGC/TACG/;
	}
	
	my @aAln = split(//,$read2Alignment);
	my @aR2  = split(//,$read2Seq);
	my @aR1  = split(//,$read1Seq);

	my $j = 0; my $nMM = 0; my $nGaps = 0; my $ITRstring;
	
	print STDERR "$read2Alignment\n$read2Seq\n$read1Seq\n";

	while ($nMM <= $maxMismatches && $j <= $#aR2) {
		if ($aAln[$j] eq "-"){
			$nGaps++ ;
			$ITRstring .= 'F';
		}else{
	...
	my $ITRcigar = ITRcigar($ITRstring);

	my $totalHomologyLength = $j;
		
	my $uHomologyLength = $totalHomologyLength - $nGaps ;
	
	$uHomologyLength = 0 if ($uHomologyLength < 0);

	print STDERR "hom = $totalHomologyLength  \ngap = $nGaps\nmm = $nMM\nuHom = $uHomologyLength\niCigar = $ITRcigar\n";

	return ($totalHomologyLength,$uHomologyLength,$nGaps,$ITRcigar);
}
...
sub ITRcigar {
	...
	my ($last, $length, @nCigar, $run);

	map {
	    $length           = ($aStr[$_ - 1] eq $last)? $length + 1: 1;
	    $run++            unless  $aStr[$_ - 1] eq $last;
	    $last             = $aStr[$_ - 1];
	    $nCigar[$run]     = [$aStr[$_ - 1], $length];
	} (1 .. scalar @aStr);

	my @iCigar = map { $nCigar[$_][1] . $nCigar[$_][0] } ( 1 .. $#nCigar);
