...
my (%hotspots,$NCISfactor,$probCnt);
my ($tmpStem)                      = int(rand()*10000000000);
my ($tempFlanks,$hFlanks)          = genTempFile('flanks');
my ($tempHS,$temphHS)              = genTempFile('hs');
my ($fragstmpFile,$tempFragsFileH) = genTempFile('frags');
...
my ($tfX1,$tfhX1) = genTempFile('checkSort');
my ($tfX2,$tfhX2) = genTempFile('checkSort');
...
sub recenterPeaks{
    ...
    my ($tempOut,$tempOutFh) = genTempFile('recentered');
    $tempOut = 'tempOut.bed' if ($noTmp);
    open OUT, '>', $tempOut;

    my ($tempHS3Col,$temphHS3Col) = genTempFile('recentered3Col');
    ...
    while (<$PIPE>){
        ...
        if ($lastHS{name}){
            if ($hs{name} ne $lastHS{name}){
	            printRecenteredHS(\@pos,\@neg,$lastHS{cs},$lastHS{from},$lastHS{to},$altCenter) unless ($hsDone{$hs{name}}++);
...
sub printRecenteredHS{
    ...
    my $negMed = median($pNeg);
		...
		if (!$posMed){
			print STDERR "WARNING ! Hotspot lacks FWD/LEFT reads !\n";
			print STDERR join("\t",$pCS,$pFrom,$pTo,++$probCnt)."\n";
		}

		if (!$negMed){
			print STDERR "WARNING ! Hotspot lacks REV/RIGHT reads !\n";
			print STDERR join("\t",$pCS,$pFrom,$pTo,++$probCnt)."\n";
		}
    ...
    my $midPoint;
		...
		if (not ($altCenter)){
			if ($posMed && $negMed){     $midPoint = round(($posMed + $negMed)/2)};
    	if ($posMed && not($negMed)){$midPoint = round($statPos->percentile(80))};
    	if ($negMed && not($posMed)){$midPoint = round($statNeg->percentile(20))};
    }else{
	    if ($posMed && $negMed){     $midPoint = round(($statPos->percentile(80) + $statNeg->percentile(20))/2)};
    	if ($posMed && not($negMed)){$midPoint = round($statPos->percentile(80))};
    	if ($negMed && not($posMed)){$midPoint = round($statNeg->percentile(20))};
    }
...
sub makeFlanksFile{
	...
	my ($tf1,$tfH1) = genTempFile('makeflanks_1');
	my ($tf2,$tfH2) = genTempFile('makeflanks_2');
	...
	##################### STEP 2
	my ($tf3,$tfH3) = genTempFile('makeflanks_3');
...
sub genCoverage{
	...
	my ($coverageFileF,$tfH1) = genTempFile('coverage_1');
	my ($coverageFileR,$tfH2) = genTempFile('coverage_2');
...
sub printHS{
	...
	my ($prelimOut,$poH) = genTempFile('prelim');
	...
	my ($outTmpA,$oTA) = genTempFile('stepA');
	my ($outTmpB,$oTB) = genTempFile('stepB');
	my ($outTmpC,$oTC) = genTempFile('stepC');
	my ($outTmpD,$oTD) = genTempFile('stepD');
...
sub genTempFile {
	my ($prefix) = shift;
	$prefix = $prefix?$prefix:"tmp";
	return ($prefix."_".$tmpStem."_".int(rand()*100000000).".tmp",1);
}
